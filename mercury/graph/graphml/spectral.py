...
class SpectralClustering(BaseClustering):
    ...
    def __init__(self, n_clusters=2, mode='networkx', max_iterations=10, random_state=0):
    ...
    def _fit_networkx(self, graph: Graph):
        ...
        kmeans = KMeans(n_clusters=self.n_clusters, random_state=self.random_state).fit(U)
    ...
    def _fit_spark(self, graph: Graph):
        ...
        vertices_mapping = graph_frames_graph.vertices.withColumn("idx", F.monotonically_increasing_id())

        mapped_node_ids = graph_frames_graph.edges \
            .join(vertices_mapping, graph_frames_graph.edges.src == vertices_mapping.id) \
            .withColumnRenamed("idx", "src_mapped") \
            .drop('id', 'src')

        mapped_node_ids = mapped_node_ids \
            .join(vertices_mapping, mapped_node_ids.dst == vertices_mapping.id) \
        ...
        self.labels_ = vertices_mapping \
            .join(assignments, vertices_mapping.idx == assignments.id) \
            .drop(assignments.id) \
            .selectExpr(['id as node_id', 'cluster'])

        self.modularity_ = self._spark_modularity(graph_frames_graph.edges, graph_frames_graph.degrees)
    ...
    def _spark_modularity(self, edges, degrees, resolution=1):
        ...
        for i in range(self.n_clusters):
            ...
            l_c = edges \
                .filter(edges.src.isin(nodeids) & edges.dst.isin(nodeids)) \
                .count()

            k_c = nids \
                .join(degrees.withColumnRenamed('id', 'node_id'), on='node_id') \
                .select(F.sum('degree')) \
                .collect()[0][0]
